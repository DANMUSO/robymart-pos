...
class Language 
{
	...
	private $lang_id;
	private $data = array();
	private $lang_code; // Declare property for lang_code

	public function __construct($lang_code) 
	{
		$this->lang_code = $lang_code; // Initialize lang_code property
	...
	public function load() 
	{
		if (empty($this->data)) {
			...
			foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $value) {
				$key = isset($value['lang_key']) ? trim($value['lang_key']) : '';
				$lang_value = isset($value['lang_value']) ? trim($value['lang_value']) : '';
				$this->data[$key] = $lang_value;
			}
		}
	}
}
